...
class Sliderimages_model extends CI_Model
{
  	...
  	function __construct()
    {
    	...
    	$this->sliderimages_description = 'sliderimages_description';
    	$this->news = 'news';
    }
	
	public function save($data)
	{
		$this->db->insert($this->sliderimages,$data);
		return true;
	}
	...
	{
		$this->db->insert_batch($this->sliderimages_description,$data);
		return true;
	}
	...
	public function getAllSliderImages(){
		...
		$this->db->from($this->sliderimages.' m');
		$query = $this->db->get();
		if($query->num_rows())
		{
			return $query->result();
	...
	public function getHomePageSliderImages(){
		$this->db->select('m.*,md.*');
		$this->db->join($this->sliderimages_description.' md','md.slider_id = m.id');
		$this->db->where('md.language_id',getLangId());
		$this->db->limit(3,0);
	...
	{
		$this->db->where('id',$id);
		$this->db->update($this->sliderimages,$data);
		return true;
	}
	...
	public function updateSpeical($data,$id)
	{
	 ...
	 public function update_sliderimages_description($data,$id){
		...
		$this->db->update_batch($this->sliderimages_description,$data,'language_id');
		return true;
	}
	...
	public function deleteSliderFile($id){
		...
		if($row['picture']!=''){
			...
			$data = array("picture" => "");
			$this->db->where('id',$id);
			$this->db->update('sliderimages',$data);
		}
	}
